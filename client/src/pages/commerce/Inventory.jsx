...
const Inventory = () => {
  ...
  const columns = [
      ...
      render: (_, record) => (
        <div className="w-32">
          <div className="flex justify-between mb-1">
            <span
              className={`text-[10px] font-bold ${record.stock === 0 ? "text-red-500" : "text-emerald-500"}`}
            >
    ...
    },
    {
      ...
      dataIndex: "price",
      render: (price) => (
        <span className="font-black text-slate-800 text-[13px]">{price}</span>
      ),
    },
    {
      title: "CATEGORY",
      dataIndex: "category",
      render: (cat) => (
        <Tag className="border-none bg-slate-100 text-slate-500 font-bold text-[10px] px-3 rounded-full uppercase tracking-tighter">
          {cat}
        </Tag>
      ),
    },
  ...
  return (
    <div className="p-8 bg-[#f8fafc] min-h-screen font-['Urbanist']">
      ...
      <div className="flex justify-between items-center mb-10">
        <div className="flex items-center gap-3">
          <h1 className="text-[22px] font-black text-[#1e293b] tracking-tight uppercase">
            Inventory Management
          </h1>
          <Tag
            color="blue"
            className="rounded-md font-bold text-[10px] border-none bg-blue-50 text-blue-500"
          >
            V2.4.0
          </Tag>
        </div>
        <Space size="middle">
          <Button
            icon={<PlusOutlined />}
            type="primary"
            className="bg-[#10b981] border-none h-10 px-6 rounded-xl font-bold hover:opacity-90 transition-all"
          >
            Add Product
          </Button>
      ...
      <div className="grid grid-cols-3 gap-8 mb-10">
        {cards.map((card, i) => (
          ...
          >
            ...
            <div className="text-4xl font-black text-[#0f172a] mb-5">
              {card.value}
            </div>
            <div className="flex items-center gap-2">
              <Tag
                color="green"
                className="border-none bg-green-50 text-green-500 font-black rounded-full px-3 text-[11px]"
              >
                {card.change}
              </Tag>
      ...
      <div className="bg-white p-6 rounded-t-[32px] border border-gray-100 border-b-0 flex items-center justify-between gap-4">
        ...
        <Space size="middle">
          ...
          <Button
            icon={<FilterOutlined />}
            className="h-12 w-12 rounded-2xl border-gray-100 shadow-sm flex items-center justify-center"
          />
          <Button
            icon={<DownloadOutlined />}
            className="h-12 px-6 rounded-2xl border-gray-100 shadow-sm font-bold text-xs text-slate-600 hover:text-emerald-500"
          >
            Export CSV
          </Button>
        </Space>
      ...
      <div className="bg-white rounded-b-[32px] border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="p-6 border-t border-gray-50 flex justify-between items-center bg-white">
          ...
          <div className="flex gap-2">
            <Button
              size="small"
              className="rounded-lg font-bold text-[10px] hover:text-emerald-500"
            >
              PREV
            </Button>
            <Button
              size="small"
              type="primary"
              className="bg-[#10b981] border-none rounded-lg font-bold text-[10px]"
            >
              1
            </Button>
            <Button
              size="small"
              className="rounded-lg font-bold text-[10px] hover:text-emerald-500 border-gray-100"
            >
              2
            </Button>
            <Button
