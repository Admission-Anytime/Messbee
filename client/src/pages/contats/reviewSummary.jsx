        ...
        return (
          <div key={step.number} className="flex items-center">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className={`text-[10px] font-semibold uppercase tracking-wider ${isActive ? "text-green-600" : isCompleted ? "text-green-500" : "text-gray-400"}`}>Step {step.number}</p>
                <p className={`text-xs font-bold ${isActive ? "text-gray-900" : isCompleted ? "text-gray-600" : "text-gray-400"}`}>{step.label}</p>
              </div>
            </div>
            {!isLast && <div className={`h-0.5 w-48 sm:w-64 lg:w-80 mx-4 rounded-full ${isCompleted ? "bg-green-500" : "bg-gray-200"}`}/>}
          </div>
        );
      })}
    </div>
...
const CircularProgress = ({ percentage }) => {
  ...
  useEffect(() => {
    ...
    const animate = (ts) => {
      ...
      if (progress < 1) requestAnimationFrame(animate); else setPct(percentage);
    };
    requestAnimationFrame(animate);
  }, [percentage]);
...
export default function ReviewSummary() {
  ...
  const FAIL_ROWS  = importData.failedRows ?? [];
  const PERCENTAGE = TOTAL > 0 ? (SUCCESSFUL / TOTAL) * 100 : 0;

  const [showAll, setShowAll] = useState(false);
  const displayed = showAll ? FAIL_ROWS : FAIL_ROWS.slice(0, 4);

  // Redirect guard: if someone lands here without import state, send them back
  useEffect(() => {
    if (!location.state) navigate("/admin/contacts/import", { replace: true });
  ...
  const handleDownloadError = () => {
    const csv = ["Row ID,Entry Data,Name,Reason", ...FAIL_ROWS.map(f => `${f.row},"${f.data}","${f.name}","${f.reason}"`)].join("\n");
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
          <div className="flex flex-col items-center py-10 px-8">
            ...
            <p className="text-sm text-gray-500 text-center max-w-sm">
              ...
              <span className="font-bold text-gray-800">{importData.fileName}</span>.{" "}
        ...
        {FAIL_ROWS.length > 0 && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-6">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={handleDownloadError}
