...
const Label = () => {
  ...
  // --- Naya Custom Toast Design (Screenshot jaisa) ---
  const showSuccessToast = (msg) => {
    toast.custom((t) => (
      <div className={`${t.visible ? 'animate-enter' : 'animate-leave'} max-w-md w-full bg-white shadow-lg rounded-lg pointer-events-auto flex ring-1 ring-[#22C55E] border-l-8 border-[#22C55E]`}>
        <div className="flex-1 w-0 p-4">
          <div className="flex items-center">
            ...
            <div className="ml-3 flex-1">
              <p className="text-sm font-bold text-gray-900">{msg}</p>
            </div>
          </div>
        </div>
      </div>
  ...
  const handleSave = async () => {
    if (!labelName.trim()) return toast.error("Please enter a label name");
    ...
    try {
      if (editingLabel) {
        await axios.put(`http://localhost:5000/api/labels/${editingLabel._id}`, labelData);
        showSuccessToast("Label updated successfully!");
      } else {
        await axios.post('http://localhost:5000/api/labels', labelData);
  ...
  const executeDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/labels/${labelToDelete}`);
  ...
  return (
    <div className="p-4 md:p-6 bg-[#F9FAFB] min-h-screen font-sans antialiased text-gray-900">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center bg-blue-50 border border-blue-100 px-4 py-1.5 rounded-full">
            ...
            <span className="text-blue-700 font-semibold text-xs whitespace-nowrap">Total labels: {labels.length}</span>
          </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-x-auto shadow-sm">
        {loading ? (
          <div className="p-10 text-center text-gray-500">Loading labels...</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {labels.map((label) => (
                <tr key={label._id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-500 text-[13px] font-medium leading-relaxed">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-gray-700 text-[13px] font-semibold">{label.creator || 'User'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-3 text-gray-400">
                      ...
                      <button onClick={() => confirmDelete(label._id)} className="hover:text-red-500 transition-colors">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[440px] max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white flex justify-between items-center px-6 py-4 border-b z-10">
              ...
              <button onClick={closeModal} className="text-gray-400 hover:text-gray-600 text-2xl font-light leading-none">&times;</button>
            </div>
            
            <div className="p-6 space-y-5">
              <div>
                <label className="text-[13px] font-bold text-gray-700 block mb-1.5">Label Name</label>
                <input type="text" value={labelName} onChange={(e) => setLabelName(e.target.value)} placeholder="e.g. VIP Customer" className="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-sm font-medium bg-gray-50/30" />
              </div>

              <div>
                <label className="block text-[13px] font-bold text-gray-700 mb-1.5">Description</label>
                <textarea rows="3" value={labelDesc} onChange={(e) => setLabelDesc(e.target.value)} placeholder="Add a description..." className="w-full border border-gray-200 rounded-xl px-4 py-2.5 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 outline-none text-sm font-medium resize-none bg-gray-50/30" />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-6 gap-3 mb-6">
                  {colorPalette.map((color) => (
                    <button key={color} onClick={() => setSelectedColor(color)} className={`aspect-square rounded-full transition-transform ${selectedColor === color ? 'ring-2 ring-offset-2 ring-emerald-500 scale-110 shadow-sm' : 'hover:scale-105 shadow-inner'}`} style={{ backgroundColor: color }} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="sticky bottom-0 bg-white flex justify-end items-center gap-3 px-6 py-4 border-t">
              <button onClick={closeModal} className="text-[13px] font-bold text-gray-500 hover:text-gray-700 px-4 py-2">Cancel</button>
              <button onClick={handleSave} className="bg-[#10B981] hover:bg-[#059669] text-white px-6 py-2.5 rounded-xl font-bold text-[13px] shadow-md">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[70] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[360px] p-6 text-center">
            ...
            <h3 className="text-[18px] font-bold text-gray-800 mb-2">Delete Label?</h3>
            <p className="text-[13px] font-medium text-gray-500 mb-8 px-2">This action cannot be undone.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <button onClick={() => setIsDeleteModalOpen(false)} className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-[13px] font-bold text-gray-600">Cancel</button>
              <button onClick={executeDelete} className="flex-1 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-xl text-[13px] font-bold shadow-md">Confirm</button>
            </div>
          </div>
        </div>
      )}
