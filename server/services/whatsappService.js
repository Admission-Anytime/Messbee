...
class WhatsAppService {
  ...
  /**
   * Validate Graph API configuration that depends on a WABA.
   */
  validateBusinessConfig() {
    this.validateConfig();

    if (!this.businessAccountId) {
      throw new Error('WhatsApp business account configuration is incomplete. Please check WHATSAPP_BUSINESS_ACCOUNT_ID in .env file');
    }
  ...
  /**
   * Shared Graph API headers.
   */
  getGraphHeaders(extraHeaders = {}) {
    return {
   ...
   * Based on the collection endpoint: GET /{WABA-ID}
   */
  async getWabaDetails() {
    this.validateBusinessConfig();

    const response = await axios.get(
      `https://graph.facebook.com/${this.apiVersion}/${this.businessAccountId}`,
      {
  ...
  /**
   * Fetch phone numbers for the configured WABA.
   * Based on the collection endpoint: GET /{WABA-ID}/phone_numbers
   */
  async getPhoneNumbers() {
    this.validateBusinessConfig();

    const response = await axios.get(
      `https://graph.facebook.com/${this.apiVersion}/${this.businessAccountId}/phone_numbers`,
      {
        params: {
  ...
  /**
   * Fetch apps subscribed to the configured WABA.
   * Based on the collection endpoint: GET /{WABA-ID}/subscribed_apps
   */
  ...
  /**
   * Fetch commerce settings for the configured business phone number.
   * Based on the collection endpoint: GET /{Phone-Number-ID}/whatsapp_commerce_settings
   */
  async getCommerceSettings() {
    this.validateConfig();

    const response = await axios.get(
      `https://graph.facebook.com/${this.apiVersion}/${this.phoneNumberId}/whatsapp_commerce_settings`,
      {
        headers: this.getGraphHeaders()
      }
    );
  ...
  /**
   * Initialize or update commerce settings for the configured business phone number.
   * Based on the collection endpoint: POST /{Phone-Number-ID}/whatsapp_commerce_settings
   */
  async updateCommerceSettings({
    ...
    const response = await axios.post(
      ...
      {
        params: {
          is_cart_enabled: Boolean(isCartEnabled),
          is_catalog_visible: Boolean(isCatalogVisible),
        },
        headers: this.getGraphHeaders()
      }
    );
  ...
  /**
   * Debug current access token against Graph API.
   * Based on the collection endpoint: GET /debug_token?input_token=...
   * This check is best-effort and does not block verification if Graph rejects it.
   */
  ...
  /**
   * Verify current Meta/WhatsApp Cloud setup before inventory mutations.
   * Uses the same Graph endpoints surfaced in the shared Postman collection.
   */
  async verifyInventoryMetaSetup(options = {}) {
    const { requireCatalogVisible = false } = options;
    ...
    try {
      const tokenDebug = await this.debugAccessToken();
      checks.debugToken = {
        success: true,
        data: tokenDebug?.data || tokenDebug
      };
    } catch (error) {
      checks.debugToken = {
        success: false,
    ...
    try {
      const waba = await this.getWabaDetails();
      checks.waba = {
        success: true,
        id: waba?.id,
        name: waba?.name || null
      ...
      return {
        success: false,
        message: 'Meta verification failed while validating WhatsApp Business Account',
        checks: {
          ...checks,
          waba: {
            success: false,
    ...
    try {
      ...
      const phoneNumbers = Array.isArray(phoneNumbersResponse?.data) ? phoneNumbersResponse.data : [];
      const matchedPhoneNumber = phoneNumbers.find(
        (phone) => String(phone?.id) === String(this.phoneNumberId)
      );

      if (!matchedPhoneNumber) {
        return {
          success: false,
          message: 'Configured phone number is not linked to the configured WABA on Meta',
          checks: {
            ...checks,
            phoneNumbers: {
              success: false,
              availablePhoneNumberIds: phoneNumbers.map((phone) => phone?.id).filter(Boolean)
            }
          }
        };
      }
      ...
      return {
        success: false,
        message: 'Meta verification failed while validating WABA phone numbers',
        checks: {
          ...checks,
          phoneNumbers: {
            success: false,
    ...
    try {
      const commerceSettingsResponse = await this.getCommerceSettings();
      const commerceSettings = Array.isArray(commerceSettingsResponse?.data)
        ? commerceSettingsResponse.data[0]
        : null;

      if (!commerceSettings) {
        return {
          success: false,
          message: 'Meta verification failed because commerce settings could not be resolved',
          checks: {
            ...checks,
            commerceSettings: {
              success: false,
              error: 'No commerce settings returned by Meta'
            }
          }
        };
      }

      checks.commerceSettings = {
        success: true,
        id: commerceSettings.id || null,
        isCartEnabled: Boolean(commerceSettings.is_cart_enabled),
        isCatalogVisible: Boolean(commerceSettings.is_catalog_visible)
      };

      if (requireCatalogVisible && !commerceSettings.is_catalog_visible) {
        return {
          success: false,
          message: 'Meta commerce catalog visibility is disabled for this WhatsApp phone number',
          checks
        };
      }
    } catch (error) {
      ...
      return {
        success: false,
        message: 'Meta verification failed while validating commerce settings',
        checks: {
          ...checks,
          commerceSettings: {
            success: false,
    ...
    try {
      const subscribedAppsResponse = await this.getSubscribedApps();
      const subscribedApps = Array.isArray(subscribedAppsResponse?.data)
        ? subscribedAppsResponse.data
        : [];

      checks.subscribedApps = {
        success: true,
        count: subscribedApps.length,
        isSubscribed: subscribedApps.length > 0
      };
    } catch (error) {
      checks.subscribedApps = {
        success: false,
    ...
    return {
      success: true,
      message: 'Meta WhatsApp Cloud API verification passed',
      checks
    };
  }
